...
public sealed record SourceProfileSummary(string Name, string Adapter, bool Enabled, bool Optional, string Details, bool IsDisabledByMissingSession)
{
    ...
    public static SourceProfileSummary From(JobSourceOptions source, bool hasGlassdoorSession)
    {
        ...
        var details = adapter switch
        {
            ...
            "Drushim" when source.DrushimFilter is { } filter => $"{(filter.CategoryIds.Count > 0 ? filter.CategoryIds.Count : filter.CategoryId > 0 ? 1 : 0)} categories | {filter.SubcategoryIds.Count} subcategories | {filter.AreaIds.Count} areas | {filter.MaxDetailsPerSearch} detail pages | {(string.IsNullOrWhiteSpace(filter.Query) ? "no query" : "query")}",
